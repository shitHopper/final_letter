...
- 可选信件密码保护（4–16 位，查看时需验证）
- 编辑时可清除密码保护（勾选「清除密码保护」复选框，发送 password: null）
- 推送方式：
...
- 密码 4–16 位，服务端 scrypt 哈希存储（16 字节随机盐）
...
- 信件访问 Token 持久化到 letter_verify_tokens 数据库表（防重启丢失），一次性使用（验证后删除），防重放
...
**阶段3 — 清理**：删除过期的邮箱验证码记录和过期的信件验证 Token 记录。
...
### letter_verify_tokens（信件验证 Token）
| 字段 | 类型 | 说明 |
|------|------|------|
| letter_id | INTEGER PK | 信件 ID |
| token | TEXT | 随机 Token（32 字节 hex） |
| expires_at | TEXT | 过期时间 |
...
### 当前 Demo 阶段
- 前端：React 19 + Vite 8 SPA，共享工具函数（`src/utils.js`，含 `parseUTC` 等）
...
- 前端校验：密码 4-16 位（前后端一致）、头像上传前端格式/大小预校验
